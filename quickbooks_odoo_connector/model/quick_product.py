    ...
    def product_import_mapper(self, backend_id, data):
        ...
        if 'Item' in record:
            ...
            if rec['Type'] == 'Category':
                category_id = self.env['product.category'].search(
                    [('quickbook_id', '=', rec['Id']), ('backend_id', '=', backend_id)])
                vals = {
                    'name': rec.get('Name'),
                    'backend_id': backend_id,
                    'quickbook_id': rec.get('Id'),
                }
                if not category_id:
                    return self.env['product.category'].create(vals)
                else:
                    category = category_id.write(vals)
                    return category
        ...
        if 'ParentRef' in rec:
            categ_id = self.env['product.category'].search(
                        [('quickbook_id', '=', rec.get('ParentRef').get('value')), ('backend_id', '=', backend_id)])
            if categ_id:
