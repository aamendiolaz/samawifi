...
class quickbook_journal_entry(models.Model):
    ...
    def journal_mapper(self, backend_id, data):
        record = data
        ...
        if 'Term' in record:
            ...
            lines_pay_ids = []
            if 'Type' in rec:
                term = self.env['account.journal'].search(
        ...
        pay_term_id = self.env['account.journal'].search(
    ...
    def journal_batch(self, model_name, backend_id, filters=None):
        """ Import term Details. """
        arguments = 'term'
        count = 1
        record_ids = ['start']
        ...
        if record_ids:
            if 'Term' in record_ids['QueryResponse']:
                ...
                for record_id in record_ids:
                    self.env['account.journal'].importer(arguments=arguments, backend_id=backend_id,
                                                                           filters=filters,
    ...
    def get_ids_new(self, arguments, backend_id, filters, record_id):
        ...
        if not record_id:
            data = headeroauth.get(backend.location+backend.company_id +
                                        method+arguments+'%20STARTPOSITION%20'+str(filters['count'])+'%20MAXRESULTS%20' + str(300)+'&minorversion=4', headers=headers)
        else:
            data = headeroauth.get(backend.location + backend.company_id +
                                   '/' + arguments + '/' + str(record_id) + '?minorversion=4', headers=headers)
            if data.status_code == 429:
    ...
    def journal_mapper_new(self, backend_id, data):
        ...
        if not account_journal_id:
            ...
            account = account_journal_id.write(vals)
            return account