...
class CustomAccountInvoice(models.Model):
    ...
    discount_rate = fields.Float('Discount Amount', default=0.0, digits=(16, 2),store=True,readonly=True,
                                 states={'draft': [('readonly', False)]})
    amount_discount = fields.Monetary(default=0.0, string='Discount', store=True, readonly=True, compute='_compute_discount',
        ...
        for i in self:
            if i.discount_type:
                if i.discount_rate < 0.0:
                    ...
                    i.amount_discount = i.discount_rate
            else:
                i.amount_discount = 0.0
                i.discount_rate = 0.0
    ...
    def _compute_amount(self):
        for move in self:
            ...
            move.amount_residual = move.amount_residual - move.amount_discount

            move.amount_untaxed_signed = -total_untaxed
            move.amount_tax_signed = -total_tax

            move.amount_total_signed = abs(total) if move.move_type == 'entry' else -total
            move.amount_total_signed = move.amount_total_signed - move.amount_discount

            move.amount_residual_signed = total_residual
            move.amount_residual_signed = move.amount_total_signed - move.amount_discount
...
class UpdateAccountPaymentRegister(models.TransientModel):
    _inherit = 'account.payment.register'

    def _get_wizard_values_from_batch(self, batch_result):
        res = super(UpdateAccountPaymentRegister, self)._get_wizard_values_from_batch(batch_result)
        ...
        #minus the discount amount from source_amount
        for i in invoices:
            source_amount = source_amount - i.amount_discount
